...
function SortableItem({ item, onDelete }: { item: Item; onDelete: (value: Item) => void }) {
      ...
      <div className="flex items-center gap-2 px-2 py-1 border-0 ring-1 ring-inset ring-gray-300 rounded-md bg-white">
        <div className="flex items-center relative gap-2">
          <span
            ...
            {...listeners}
          />
          <img src={`/topics/${item.id}/photo`} alt="" className="h-6 w-6 flex-shrink-0 rounded-full" />
...
function TopicsComboBox({ options, onChange }: { options: Item[]; onChange: (value: Item) => void }) {
  const [query, setQuery] = useState('');
  const [selectedItem, setSelectedItem] = useState<Item | null>(null);

  const handleChange = (topic: Item) => {
    setQuery('');
